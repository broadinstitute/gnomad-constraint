...
setwd("/Users/kristen/Projects/constraint/plots")
...
####################################################################
####################################################################
v2_constraint_data <- read.delim("gnomad.v2.1.1.lof_metrics.by_gene.txt.bgz")
v4_constraint_data <- read.delim("extra_annotations.tsv") # gnomad v4.0 file, will replace when v4.1 is ready
...
# TODO: Convert hard-coded paths to options
list_dir = '/Users/kristen/code/broad/gene_lists/lists'
...
gene_lists <- gene_lists %>% 
  ...
  mutate(gene_list = fct_recode(gene_list, 'Autosomal Recessive' = "all_ar", 'Autosomal Dominant' = "all_ad")) 
...
plot_gene_lists <- function(df, version){
  ...
  df <- df %>% filter(!is.na(gene_list) & !is.na(!!sym(metric))) %>%
  ...
  gene_list_sums <- df %>% group_by(metric, gene_list, .drop = FALSE) %>% summarise(count = n())
  ...
  props <- gene_list_sums %>% group_by(gene_list) %>% mutate(prop_in_bin = count/sum(count))
  ...
  ggsave(bar_plot, filename = paste("gene_list_barplot_", version, ".png", sep=""), dpi = 300, width = 11, height = 6, units = "in")
  
}

plot_gene_lists(gene_data, "v2")
plot_gene_lists(gene_data, "v4")
...
plot_roc <- function(df, metric){
  ...
  ggsave(roc_plot, filename = paste("roc_plot_", title_label, ".png", sep=""), dpi = 300, width = 6, height = 6, units = "in")
}

plot_roc(constraint_data, "loeuf")
