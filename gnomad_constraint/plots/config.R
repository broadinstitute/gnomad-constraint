...
get_or_download_file <- function(base_fname, version = "v4.0", output_path = default_output_path, data_dir = default_data_dir, subfolder = "", local_name = "") {
  local_path <- paste0(output_path, "/", data_dir, "/", ifelse(local_name != "", local_name, base_fname))
  print(local_path)
  if (version == "v2.1.1") {
  ...
  if (!file.exists(local_path)) {
    ...
    gcs_get_object(strsplit(remote_path, paste0(data_bucket, "/"), fixed = TRUE)[[1]][2], data_bucket, saveToDisk = local_path)
  }
...
load_all_gene_list_data <- function(output_path = default_output_path, gene_lists = default_gene_lists) {
  ...
  if (length(all_files) == 0) {
    if (length(all_files) == 0) system(paste0("git clone https://github.com/macarthur-lab/gene_lists.git ", list_dir))
    all_files <- list.files(list_dir, ".+tsv")
  }
  gene_lists <- map_df(
    all_files,
