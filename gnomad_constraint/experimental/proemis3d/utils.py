...
def determine_regions_with_min_oe_upper(
    ...
    min_oe_upper_expr = get_3d_residue(
        ht.aa_index,
        ht.dist_mat,
        ht.oe,
        alpha=alpha,
    ...
    if debug:
        debug_output = min_oe_upper_expr[1]
    ...
    if debug:
        debug_output += debug_print_oe_and_regions(
            ht,
            title="determine_regions_with_min_oe_upper: Final output",
        )
        return ht, debug_output
...
def _write_debug_output(debug_output_file: str, debug_output_str: str):
    """Write debug output to file, handling both local and GCS paths."""
    if debug_output_file.startswith("gs://"):
        # GCS path - use Hail's hadoop_open
        with hl.hadoop_open(debug_output_file, "wb") as f:
            f.write(debug_output_str.encode("utf-8"))
    else:
        # Local path
        with open(debug_output_file, "w", encoding="utf-8") as f:
            f.write(debug_output_str)
