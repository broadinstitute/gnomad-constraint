...
from gnomad.utils.constraint import get_all_pop_lengths
...
gene_lof_matrix_path = (
    "gs://gnomad-julia/v4.0/constraint/individual_level/full_gene_lof_matrix_{}.mt"
)
homozygous_lof_mt_path_raw = "gs://gnomad-julia/v4.0/constraint/individual_level/all_homozygous_lofs{data_type}.mt"
lofs_by_gene_ht_path_raw = f"{root}/homozygous_lof_summary{{data_type}}.ht"
coverage_summary_ht_path = f"{root}/coverage_summary.ht"

raw_constraint_ht_path = (
    f"gs://gnomad/v4.0/constraint/metrics/gnomad.v4.0.constraint_metrics.ht"
)
...
def create_coverage_summary(overwrite: bool = False):
    ...
    ht = constraint_res.get_coverage_ht("exoomes", "4.0").ht()
    ht = ht.annotate(cds=gtf.index(ht.locus, all_matches=True)).explode("cds")
    ht = ht.group_by(
        gene_id=ht.cds.gene_id, transcript_id=ht.cds.transcript_id
    ).aggregate(
        mean_coverage=hl.agg.mean(ht.mean),
        median_coverage=hl.agg.approx_quantiles(ht.median, 0.5),
    )
    return ht.checkpoint(
        coverage_summary_ht_path, overwrite=overwrite, _read_if_exists=not overwrite
    )
...
def get_an_adj_criteria(
    mt,
    samples_by_sex: Optional[dict] = None,
    an_adj_proportion_cutoff: float = AN_ADJ_FILTER,
):
    ...
    return (
        ...
        .when(
            ...
            mt.freq[0].AN
            >= an_adj_proportion_cutoff * 2 * sum(samples_by_sex.values()),
        )
        .when(
            mt.locus.in_x_nonpar(),
            mt.freq[0].AN
            >= an_adj_proportion_cutoff
            * (samples_by_sex["XY"] + samples_by_sex["XX"] * 2),
        )
        .when(
            mt.locus.in_y_nonpar(),
            mt.freq[0].AN >= an_adj_proportion_cutoff * samples_by_sex["XY"],
        )
        .or_missing()
    )


def generate_gene_lof_matrix(
    mt: hl.MatrixTable,
    filter_an_adj: bool = False,
    common_var_filt: bool = False,
    ...
    annotation_expr["mane_select"] = hl.is_defined(mt.lof_csqs.mane_select)

    mt = mt.annotate_rows(**annotation_expr)
    mt = mt.checkpoint(
        "gs://gnomad-tmp-4day/julia/gene_lof_matrix_pre_agg.mt",
        overwrite=False,
    ...
    return (
        ...
        .aggregate_rows(
            ...
            defined_sites=hl.agg.sum(mt.freq[0].AN),
            defined_sites_raw=hl.agg.sum(mt.freq[1].AN),
        )
        .aggregate_entries(
...
def generate_gene_lof_summary(mt, collapse_indels: bool = False):
    if collapse_indels:
        grouping = ["gene_id", "gene"]
        grouping.extend(["transcript_id", "mane_select"])
        mt = (
    ...
    ht = mt.annotate_rows(
        no_lofs=hl.agg.count_where(
            (mt.defined_sites > 0) & (mt.num_homs + mt.num_hets == 0)
        ),
...
def combine_lof_metrics(gene_lof_matrix):
    keys = ["gene"]
    caf_keys = ["gene"]
    caf_drop = ["gene_id", "oe"]
    constraint_path = raw_constraint_ht_path
    ht = hl.read_table(constraint_path)
    keys.append("transcript")
    caf_keys.append("transcript_id")
    caf_drop.append("mane_select")

    constraint_ht = ht.drop(
        *[x for x in ht.row if x.endswith("_classic") or x.endswith("_with_os")]
    )
    constraint_ht = add_rank(constraint_ht, "oe_lof_upper", defined_only=True).key_by(
        *keys
    )
    caf_ht = hl.read_table(gene_lof_matrix.replace(".mt", ".summary.ht"))
    mapping = caf_ht.freq_index_dict.collect()[0]
    caf_dict = {
        f"classic_caf_{pop}": caf_ht.classic_caf_array[mapping[f"gnomad_{pop}"]]
        for pop in map(lambda x: x.lower(), EXOME_POPS)
    }
    caf_dict.update(
    ...
    constraint_ht = constraint_ht.annotate(
        **caf_ht[constraint_ht.key],
        **gene,
        **coverage,
    )
    # If CAF data is missing and LoFs are possible in the gene, set all CAF
    # metrics to zero
    constraint_ht = constraint_ht.annotate(
        **{
            x: hl.cond(
                hl.is_missing(constraint_ht[x]) & (constraint_ht.possible_lof > 0),
                0,
                constraint_ht[x],
            )
            for x in list(caf_ht.row)
            if x in constraint_ht.row
            and constraint_ht[x].dtype
            in {hl.tint32, hl.tfloat32, hl.tint64, hl.tfloat64}
        }
    )
    return constraint_ht.annotate(
        # **expr_ht[constraint_ht.gene_id]
    ).annotate_globals(
        freq_meta=caf_ht.index_globals().freq_meta,
        freq_index_dict=caf_ht.index_globals().freq_index_dict,
    )
...
def find_in_meta(downsampling, pop, freq_meta):
    for i, x in enumerate(freq_meta):
        if (
            x.get("group") == "adj"
            and x.get("pop") == pop
            and x.get("downsampling") == str(downsampling)
        ):
            return i
...
def explode_downsamplings(ht, select_mane_select=True):
    pop_lengths = get_all_pop_lengths(ht, prefix="exp_lof_")
    downsamplings = list(map(lambda x: x[1], get_downsamplings(ht)))
    freq_meta = ht.freq_meta.collect()[0]
    meta_locs = [
        (i, pop, downsamplings[i], find_in_meta(downsamplings[i], pop, freq_meta))
        for count, pop in pop_lengths
        for i in range(count)
    ]
    fields = ["data"]
    if select_mane_select:
        fields.insert(0, "mane_select")
    ht = ht.annotate(
        data=[
            hl.struct(
                pop=pop,
                downsampling=downsampling,
                exp_syn=ht[f"exp_syn_{pop}"][i],
                obs_syn=hl.cond(
                    hl.is_missing(ht[f"obs_syn_{pop}"][i]) & hl.is_defined(ht.obs_syn),
                    0,
                    ht[f"obs_syn_{pop}"][i],
                ),
                exp_mis=ht[f"exp_mis_{pop}"][i],
                obs_mis=hl.cond(
                    hl.is_missing(ht[f"obs_mis_{pop}"][i]) & hl.is_defined(ht.obs_mis),
                    0,
                    ht[f"obs_mis_{pop}"][i],
                ),
                exp_lof=ht[f"exp_lof_{pop}"][i],
                obs_lof=hl.cond(
                    hl.is_missing(ht[f"obs_lof_{pop}"][i]) & hl.is_defined(ht.obs_lof),
                    0,
                    ht[f"obs_lof_{pop}"][i],
                ),
    ...
    return oe_confidence_interval(
        ht, ht.obs_lof, ht.exp_lof, prefix="oe_lof", select_only_ci_metrics=False
    )
...
def compute_homozygous_lof(mt, tx_ht):
    ...
    mt = mt.filter_rows(
        (mt.vep.worst_csq_by_gene.lof == "HC")
        & hl.is_missing(mt.vep.worst_csq_by_gene.lof_flags)
    )
    mt = mt.filter_rows(
        hl.agg.any(mt.GT.is_hom_var() & (mt.AD[1] / hl.sum(mt.AD) >= 0.8))
    )
...
def main(args):
    ...
    all_lof_metrics_path = f"{root}/{subdir}gnomad.v4.0.lof_metrics.{{extension}}"
    ...
    if args.calculate_gene_lof_matrix:
        ...
        mt = vds.variant_data
        mt = filter_to_adj(mt)
        mt = mt.select_entries("LA", "LGT")
        mt = hl.experimental.sparse_split_multi(mt, filter_changed_loci=True)
        ht = constraint_res.get_sites_resource("exomes", version).ht()
        vep_ht = hl.read_table(
            "gs://gnomad/v4.0/annotations/exomes/gnomad.exomes.v4.0.vep.ht"
        )
        mt = mt.annotate_rows(**ht[mt.row_key], vep=vep_ht[mt.row_key].vep)
        mt = generate_gene_lof_matrix(
            mt,
            filter_an_adj=args.filter_an_adj,
            pre_loftee=args.no_loftee,
    ...
    if args.export_gene_lof_matrix:
        ...
        ht = generate_gene_lof_summary(mt, not args.dont_collapse_indels)
        ...
        hl.read_table(gene_lof_matrix.replace(".mt", ".summary.ht")).drop(
            "classic_caf_array"
        ).export(gene_lof_matrix.replace(".mt", ".summary.txt.bgz"))
    ...
    if args.combine_lof_metrics:
        ht = combine_lof_metrics(gene_lof_matrix)
        ht.write(
            all_lof_metrics_path.format(extension="by_transcript.ht"), args.overwrite
        )

    if args.export_combined_metrics:
        ht = hl.read_table(all_lof_metrics_path.format(extension="by_transcript.ht"))
        ht = ht.annotate(
            constraint_flag=hl.delimit(ht.constraint_flag, "|"),
        ...
        ht = (
            add_rank(ht, "oe_lof_upper", defined_only=True)
            .drop("mane_select")
            .key_by("gene_id", "gene")
        )
        ...
        ht.export(
            all_lof_metrics_path.format(extension="downsamplings.txt.bgz").replace(
                "v1.0", "v1.1"
            )
        )

    data_type = "genomes" if args.genomes else "exomes"
    homozygous_lof_mt_path = homozygous_lof_mt_path_raw.format(
        data_type="_genomes" if args.genomes else ""
    )
    ...
    lofs_by_gene_ht_path = lofs_by_gene_ht_path_raw.format(
        data_type="_genomes" if args.genomes else ""
    )
    if args.export_homozygous_lof:
        mt = hl.read_matrix_table(homozygous_lof_mt_path)
        if args.include_low_pext:
            homozygous_lof_mt_path = homozygous_lof_mt_path.replace(
                ".mt", ".low_pext.mt"
            )
        ...
        mt = mt.annotate_rows(
            info=hl.struct(
                ...
                AN=mt.freq[0].AN,
                AF=mt.freq[0].AF,
                n_hom=mt.freq[0].homozygote_count,
                CSQ=vep_ht[mt.row_key].vep,
            )
        ).drop("is_missing")
        if args.genomes:
            exome_ht = hl.read_matrix_table(
                homozygous_lof_mt_path_raw.format(data_type="")
            ).rows()
            mt = mt.filter_rows(hl.is_missing(exome_ht[mt.row_key]))

        hl.export_vcf(
            mt,
            homozygous_lof_mt_path.replace(".mt", ".vcf.bgz"),
            metadata={
        ...
        hl.read_table(lofs_by_gene_ht_path).export(
            lofs_by_gene_ht_path.replace(".ht", ".txt.bgz")
        )
...
def count_lofs(ht, data_type):
    ...
    basic_lof_csqs = ht.vep.transcript_consequences.filter(
        lambda x: lof_cats.contains(
            add_most_severe_consequence_to_consequence(x).most_severe_consequence
        )
        & (x.biotype == "protein_coding")
    )
    basic_mane_select_lof_csqs = ht.vep.transcript_consequences.filter(
        lambda x: lof_cats.contains(
            add_most_severe_consequence_to_consequence(x).most_severe_consequence
        )
        & (x.biotype == "protein_coding")
    ...
    hc_noflag_lof_csqs = ht.vep.transcript_consequences.filter(
        lambda x: (x.lof == "HC") & hl.is_missing(x.lof_flags)
    )
    hc_mane_select_lof_csqs = ht.vep.transcript_consequences.filter(
        lambda x: (x.lof == "HC") & (x.mane_select == 1)
    )
    hc_noflag_mane_select_lof_csqs = ht.vep.transcript_consequences.filter(
        lambda x: (x.lof == "HC") & hl.is_missing(x.lof_flags) & (x.mane_select == 1)
    )
    if data_type == "genomes":
        sample_sizes = data_type_sex_counts["genomes"]
    else:
        sample_sizes = data_type_sex_counts["exomes"]
    result = ht.aggregate(
        hl.struct(
            basic_lofs=hl.agg.count_where(hl.len(basic_lof_csqs) > 0),
            basic_mane_select_lofs=hl.agg.count_where(
                hl.len(basic_mane_select_lof_csqs) > 0
            ),
            hc_lofs=hl.agg.count_where(hl.len(hc_lof_csqs) > 0),
            hc_noflag_lofs=hl.agg.count_where(hl.len(hc_noflag_lof_csqs) > 0),
            an_adj_hc_lofs=hl.agg.count_where(
                (hl.len(hc_lof_csqs) > 0) & get_an_adj_criteria(ht, sample_sizes)
            ),
            an_adj_hc_noflag_lofs=hl.agg.count_where(
                (hl.len(hc_noflag_lof_csqs) > 0) & get_an_adj_criteria(ht, sample_sizes)
            ),
            hc_mane_select_lofs=hl.agg.count_where(hl.len(hc_mane_select_lof_csqs) > 0),
            hc_mane_select_noflag_lofs=hl.agg.count_where(
                hl.len(hc_noflag_mane_select_lof_csqs) > 0
            ),
            an_adj_hc_mane_select_lofs=hl.agg.count_where(
                (hl.len(hc_mane_select_lof_csqs) > 0)
                & get_an_adj_criteria(ht, sample_sizes)
            ),
            an_adj_hc_mane_select_noflag_lofs=hl.agg.count_where(
                (hl.len(hc_noflag_mane_select_lof_csqs) > 0)
                & get_an_adj_criteria(ht, sample_sizes)
            ),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--by_transcript",
        help="Use all transcripts instead of worst",
        action="store_true",
    )
    parser.add_argument(
        "--filter_an_adj",
        help="Filter variants with less than 80% callrate",
        action="store_true",
    )
    parser.add_argument(
        "--no_loftee",
        help="Do not filter with LOFTEE (only for comparison)",
        action="store_true",
    )
    parser.add_argument(
        "--remove_ultra_common",
        help="Remove ultra-common (AF > 95%) variants",
        action="store_true",
    )
    parser.add_argument(
        "--dont_collapse_indels",
        help="Do not collapse indels when exporting Gene LoF matrix",
        action="store_true",
    )
    parser.add_argument(
        "--pop_specific", help="Use pop-specific matrix instead", action="store_true"
    )
    parser.add_argument(
        "--include_low_pext",
        help="Include low pext variants in export",
        action="store_true",
    )
    parser.add_argument(
        "--calculate_gene_lof_matrix",
        help="Calculate Gene LoF Matrix",
        action="store_true",
    )
    parser.add_argument(
        "--export_gene_lof_matrix", help="Export Gene LoF Matrix", action="store_true"
    )
    parser.add_argument(
        "--create_coverage_summary",
        help="Create coverage summary file",
        action="store_true",
    )
    parser.add_argument(
        "--combine_lof_metrics", help="Combine all LoF metrics", action="store_true"
    )
    parser.add_argument(
        "--export_combined_metrics",
        help="Export combined LoF metrics",
        action="store_true",
    )
    parser.add_argument(
        "--compute_homozygous_lof", help="Compute Homozygous LoFs", action="store_true"
    )
    parser.add_argument(
        "--export_homozygous_lof",
        help="Export Homozygous LoF summary",
        action="store_true",
    )
    parser.add_argument(
        "--count_lofs", help="Generate counts of LoFs for paper", action="store_true"
    )
    parser.add_argument(
        "--genomes", help="Use genomes instead of exomes", action="store_true"
    )
    parser.add_argument(
        "--slack_channel",
